...
def build_relevance_selection_prompt(subject: str, note: CollectedNote, max_passages: int = 3) -> str:
    ...
    return (
        ...
        "5. 忽略噪音：导航栏、页脚、cookie 弹窗、订阅提示、登录提示、分享按钮、Cloudflare、人机验证、浏览器检查、无关推荐、广告、空泛营销口号。"
...
def build_horizontal_analysis_prompt(subject: str, evidence_cards_json: str) -> str:
    return (
        ...
        "请先判断竞品场景：no_direct_competitor 表示没有明确直接竞品但存在间接替代方案或上一代解决方式；few_competitors 表示只有 1-2 个主要竞品；many_competitors 表示存在 3 个及以上代表性竞品。"
